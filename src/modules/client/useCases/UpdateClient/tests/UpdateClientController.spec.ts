...
describe('UpdateClient', () => {
	...
	it('should update the data of an existing client', async () => {
		...
		const createClientQuery = Object.assign(clientData, addressDataQueryPrisma)

		const createdClient = await prismaClient.client.create({
			data: createClientQuery
		})

		const body = Object.assign(clientData, {address: {city: 'testcityupdated'}})
		const clientId = createdClient.id

		const response = await request(app).put(`/clients/${clientId}`).send(body).set('Authorization', `Bearer: ${token}`)
